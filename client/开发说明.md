...
- `taskService.cjs`：通用后台长任务管理，负责跨功能耗时任务的启动、去重、active task 管理、事件推送和工作区结果落盘；不是技术方案专用服务。
...
- 每个功能的权威工作区状态保存到 `userData/workspace/<feature>.json`；运行中的任务必须由 Main 侧持续写入该文件。
...
### 流式 JSON 与修复
- Main 侧长文本 JSON 任务优先使用 `aiService.streamChat()`，并在请求中传 `response_format: { type: 'json_object' }`；这是后端到 AI 服务商的流式请求，不是 Main 到 Renderer 的流式返回。
- 流式 JSON 请求由任务代码累积 `chunk`，流结束后再解析完整 JSON；运行中只把“模型正在返回结果”等进度写入对应 workspace，避免每个 token 都落盘。
- 如果流式返回内容无法解析为 JSON，不要重新用非流式方式提交完整长文本；应把已经返回的短 JSON/近似 JSON 交给现有 JSON 修复链路修复。
- JSON 修复请求通常只包含模型已返回的 JSON 片段和错误信息，体量远小于原始文档，继续复用 `aiService.collectJsonResponse()` / `repairJsonResponse()` 的非流式修复逻辑即可，不需要改成流式。
- 只有主请求本身包含完整招标文件、投标文件、知识库全文、正文长上下文等大文本时，才需要优先考虑流式 JSON；短小配置、修复、二次校验等 JSON 请求继续使用现有 `collectJsonResponse()`。

...
当前功能模块：
- `technical-plan`：技术方案，包含标书解析、目录编辑、正文编辑的页面骨架。
- `knowledge-base`：知识库。
...
- `aiService.cjs`：OpenAI-compatible 请求封装，当前是占位。
- `fileService.cjs`：文档导入与解析，当前是占位。
...
- 所有成功、失败、警告、普通消息提示统一使用 `shared/ui/ToastProvider` 提供的 Toast；不要在页面里自建提示条、alert 或长期占位消息。
...
工具条配置优先放在 `src/app/toolbarConfig.tsx`，页面级特殊工具条可后续通过 App 层根据路由或步骤切换。
